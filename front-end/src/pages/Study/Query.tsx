...
function Query() {
  ...
  const [existingAnswerList, setExistingAnswerList] = useState(
    Array<IAnswer>(),
  );
  ...
  useEffect(() => {
    ...
    answerService
      .getAnwserByUserAndTopic(user?.id || '', id)
      .then((answerResult) => {
        ...
        questionService.getQuestionOfTopic(id).then((result) => {
          ...
          if (result.length > 0) {
            ...
            const localAnswer = answerResult.find(
              (elem) => elem.question_id === localQuestion.id,
            );
            if (localAnswer) {
    ...
    answerService
      ...
      .then(() => {
        ...
        toast.current.show({
          severity: 'success',
          summary: 'Sucesso',
          detail: 'Resposta armazenada com sucesso!',
          life: 3000,
        });
        setLoading(false);
      })
      .catch(() => {
        ...
        toast.current.show({
          severity: 'error',
          summary: 'Erro',
          detail: 'Problema ao armazenar a resposta.',
          life: 3000,
        });
        setLoading(false);
      });
  ...
  const handleChangeQuestion = (value: number) => {
    ...
    const localAnswer = existingAnswerList.find(
      (elem) => elem.question_id === localQuestion.id,
    );
