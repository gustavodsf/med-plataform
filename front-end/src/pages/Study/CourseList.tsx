    ...
    return (
      <>
        {course.topics.map((topic) => {
          return (
            <div
              className="my-topic-container"
              key={`${topic.id}_${course.id}`}
              onClick={() => handleTopicCick(topic)}
            >
              <img src={logoImg} alt="Med One" className="logo_med_one" />
              <div className="my-container">{topic.name}</div>
            </div>
          );
        })}
      </>
      ...
      {courseList.map((course) => {
        return (
          <div key={`${course.id}`}>
            ...
            <div
              className="p-d-flex p-flex-row my-div-class p-jc-between"
              key={`topic_list_${course.id}`}
            >
