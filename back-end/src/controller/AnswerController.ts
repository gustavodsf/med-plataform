...
class AnswerController {
  async add(request: Request, response: Response) {
    ...
    try {
      ...
      const answer = await answerService.addNewAnswer(
        user_id,
        course_id,
        topic_id,
        question_id,
  ...
  async getAnswerByUserTopic(request: Request, response: Response) {
    try {
      ...
      const answerService = new AnswerService();
      const answers = await answerService.getQuestionByTopicIdUserId(
        user_id,
        topic_id,
      );
      ...
      console.error(error);
      throw new Error(
        'Não foi possível localizar resposta para o usuário e tópico',
      );
    }
  }
}
