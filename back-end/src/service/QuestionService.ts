...
class QuestionService {
    ...
    async getSomeSimulatedQuestions(amount: number) {
        const questionRepository = getRepository(Question);
        const questions = await questionRepository.whereEqualTo(crs => crs.simulated, true).find();
        ...
        const filteredQuestion = _.map(numbers, (idx) => questions[idx]);
    ...
    generateRandomNumbers( amount: number, numberQuestions: number ){
        ...
        while (numbers.length < amount) {
            let random = Math.floor(Math.random() * (numberQuestions - 1 ));
